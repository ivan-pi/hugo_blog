...
## The gallery (`content/gallery/`)

The **gallery** menu entry lists interactive sketches and simulation
experiments — plain-JavaScript canvases, no libraries — rendered inside the
theme. Each artefact is one **`.html` file** (not `.md`) under
`content/gallery/`: Hugo parses the front matter and inserts the rest of the
file verbatim, so the body is ordinary HTML + `<script>`, untouched by Markdown.
...
- `assets/css/gallery.css` (linked only for this section by
  ...
  screens, `.gallery-controls` is a grid that wraps its notes on narrow
  screens, `.gallery-figures` lays inline-SVG figures side by side.
- In the scripts, size the canvas from `chart.clientWidth` (see the `resize()`
